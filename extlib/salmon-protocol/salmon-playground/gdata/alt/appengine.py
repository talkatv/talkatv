...
class AppEngineTokenStore(atom.token_store.TokenStore):
  ...
  def find_token(self, url):
    ...
    tokens = load_auth_tokens(self.user)
    logging.error("***** called w url=%s, check it out bro, self.tokens = %s",url, tokens)
    if url in tokens:
      token = tokens[url]
  ...
  def remove_token(self, token):
    ...
    """
    logging.error("Watch out, removing token %s",token)
    token_found = False
    scopes_to_delete = []
    tokens = load_auth_tokens(self.user)
  ...
  def remove_all_tokens(self):
    ...
    logging.error("Whao!  Removing all tokens!")
...
def save_auth_tokens(token_dict, user=None):
  ...
  """
  logging.info("Saving auth tokens for current user!")
  if user is None:

...
class TokenStore(object):
  ...
  def __init__(self, scoped_tokens=None):
    logging.error("Initializing token store!!!!")
  ...
  def find_token(self, url):
    ...
    """
    logging.error("***** check it out bro, self._tokens = %s",self._tokens)
    if url is None:
      return None
  ...
  def remove_token(self, token):
    ...
    """
    logging.error("Removing a token!")
    token_found = False
    scopes_to_delete = []
  ...
  def remove_all_tokens(self):
    logging.error("Removing all tokens!")
    self._tokens = {} 

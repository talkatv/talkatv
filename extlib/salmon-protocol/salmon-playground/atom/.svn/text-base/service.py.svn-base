...
import logging # TODO: Remove this after debugging
...
class AtomService(object):
  ...
  def request(self, operation, url, data=None, headers=None, 
    ...
    logging.info("In Atom base class request() method")
    ...
    logging.info(" 1. token_store(=%s).find_token(%s)=%s", self.token_store, url, self.token_store.find_token(url))  
    r = auth_token.perform_request(self.http_client, operation, url, 
        data=data, headers=all_headers)
    logging.info(" 2. token_store(=%s).find_token(%s)=%s", self.token_store, url, self.token_store.find_token(url)) 
    return r
  ...
  def Get(self, uri, extra_headers=None, url_params=None, escape_params=True):
    ...
    """
    logging.info("In atom service generic Get")
